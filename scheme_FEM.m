...
for i = 1:NL
    indx = N_c_a+(i-1)*M;
    yt(indx) = 0.25*Ds_of_cs_and_T_anode((y(indx+1) + y(indx))/2, cmax_a, T)*(y(indx+1) - y(indx))/hr_a^2;
    for j = 2:M-1
        indx = N_c_a+(i-1)*M-1+j;
        Ds_r = Ds_of_cs_and_T_anode((y(indx+1) + y(indx))/2, cmax_a, T);
        Ds_l = Ds_of_cs_and_T_anode((y(indx-1) + y(indx))/2, cmax_a, T);
    ...
    yt(indx) = -a_a^2*Gn(i)/hr_a^3 - ...
        (a_a - 0.5*hr_a)^2/hr_a^4*Ds_of_cs_and_T_anode((y(indx-1) + y(indx))/2, cmax_a, T)*(y(indx) - y(indx-1));
end

% Concentration in solid (Cathode)
for i = NR:N
    indx = N_c_c+(i-NR)*M;
    yt(indx) = 0.25*Ds_of_cs_and_T_cathode((y(indx+1) + y(indx))/2, cmax_c, T)*(y(indx+1) - y(indx))/hr_c^2;
    for j = 2:M-1
        indx = N_c_c+(i-NR)*M-1+j;
        Ds_r = Ds_of_cs_and_T_cathode((y(indx+1) + y(indx))/2, cmax_c, T);
        Ds_l = Ds_of_cs_and_T_cathode((y(indx-1) + y(indx))/2, cmax_c, T);
    ...
    yt(indx) = -a_c^2*Gn(i)/hr_c^3 - ...
        (a_c - 0.5*hr_c)^2/hr_c^4*Ds_of_cs_and_T_cathode((y(indx-1) + y(indx))/2, cmax_c, T)*(y(indx) - y(indx-1));
end
