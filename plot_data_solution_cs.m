...
subplot(2, 3, 2);
plot(r.*a_a, y0(N_c_a+M*NL/2:N_c_a+M*NL/2+M-1), '.r');
hold on;
plot(r.*a_a, y(2:(n_lines-1), N_c_a+M*NL/2:N_c_a+M*NL/2+M-1), 'k');
hold on;
plot(r.*a_a, y(n_lines, N_c_a+M*NL/2:N_c_a+M*NL/2+M-1), '.b');
xlim([0 a_a]);
title('');
xlabel('r [m]');
...
subplot(2, 3, 5);
plot(r.*a_c, y0(N_c_c+M*(N-NR)/2:N_c_c+M*(N-NR)/2+M-1), '.r');
hold on;
plot(r.*a_c, y(2:(n_lines-1), N_c_c+M*(N-NR)/2:N_c_c+M*(N-NR)/2+M-1), 'k');
hold on;
plot(r.*a_c, y(n_lines, N_c_c+M*(N-NR)/2:N_c_c+M*(N-NR)/2+M-1), '.b');
xlim([0 a_c]);
title('');
xlabel('r [m]');
